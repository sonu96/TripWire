...
The fast path for pre-confirmed payments (submitted through the x402 facilitator before the transaction lands on-chain) targets sub-100 ms end-to-end latency by skipping pipeline stages (ABI decoding, finality polling) and fires a `payment.pre_confirmed` event immediately. Delivery still goes through Convoy, which adds ~20-80 ms of overhead.
...
| Event type | Trigger |
|---|---|
| `payment.pre_confirmed` | The x402 facilitator has validated the ERC-3009 authorization. The transaction is not yet on-chain. Delivered on the fast path (~100 ms end-to-end, including ~20-80 ms Convoy delivery overhead). |
...
The default mode. TripWire posts the signed `WebhookPayload` JSON to your registered URL via Convoy. Convoy is the sole delivery mechanism for Execute-mode webhooks -- it owns reliability, HMAC signing, retries, delivery logs, and the dead-letter queue.

> **Note:** A direct httpx POST delivery path (bypassing Convoy for lower latency) is planned but **not yet implemented**. All Execute-mode deliveries currently go through Convoy, which adds ~20-80 ms of delivery overhead.
...
- Typical delivery latency: ~20-80 ms (Convoy overhead).
...
During a secret rotation, Convoy signs with both the old and new key (TripWire itself does not store or handle signing secrets -- see below). Accept the delivery if either signature matches. Remove the old key after the rotation window has passed.
...
- The webhook secret is generated at endpoint registration, passed to Convoy (which uses it for HMAC signing), and returned to you exactly once in the registration response. **TripWire does not store the secret in its own database** -- Convoy is the sole holder.
- Store the secret in an environment variable or a secrets manager immediately upon receiving it. Never commit it to source control.
- If you lose the secret, you cannot retrieve it. Create a new endpoint with a fresh secret and deactivate the old one.
