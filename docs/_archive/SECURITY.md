...
TripWire handles three distinct secret flows for inbound authentication and outbound webhook signing. Understanding these flows is critical for operators and developers integrating with the platform.

### Flow 1: Goldsky → TripWire (Ingest Authentication)

```
Goldsky Turbo                          TripWire
     │                                      │
...
- **Secret**: `GOLDSKY_WEBHOOK_SECRET` (env var → `SecretStr` in settings)
- **Storage**: Environment only. Never in DB.
- **Purpose**: Authenticates that ingest payloads originate from Goldsky, not an attacker.
- **Rotation**: Change the env var and update the Goldsky pipeline config simultaneously.

### Flow 2: Facilitator → TripWire (Ingest Authentication)

```
x402 Facilitator                       TripWire
     │                                      │
     │  POST /api/v1/ingest/facilitator     │
     │  Authorization: Bearer {secret}       │
     │ ────────────────────────────────────► │
     │                                      │  hmac.compare_digest(header, expected)
     │                                      │  ✅ Process pre-confirmed event
```

- **Secret**: `FACILITATOR_WEBHOOK_SECRET` (env var → `SecretStr` in settings)
- **Storage**: Environment only. Never in DB.
- **Purpose**: Authenticates that pre-settlement notifications come from the x402 facilitator.
- **Rotation**: Independent of Goldsky secret — change env var and update facilitator config.

### Flow 3: TripWire → Developer (Webhook HMAC Signing)

This is the most complex flow. The signing secret has a specific lifecycle:

```
Registration time:
─────────────────
Developer                  TripWire                    Convoy
   │                          │                           │
   │  POST /endpoints         │                           │
   │  (SIWE authenticated)    │                           │
   │ ────────────────────────►│                           │
   ...
   │                          │  { url, secret }          │
   │                          │ ─────────────────────────►│
   │                          │                           │  Convoy stores secret
   │                          │  ◄── endpoint_id          │  internally for HMAC
   │                          │                           │
   │  ◄── { endpoint_id,     │                           │
   │        webhook_secret }  │  (secret NOT stored in    │
   │                          │   TripWire's database)    │
...
Delivery time:
──────────────
Onchain event              TripWire                    Convoy                Developer
     │                        │                          │                      │
     │  Goldsky webhook       │                          │                      │
     │ ──────────────────────►│                          │                      │
...
**Key properties:**
- The secret is generated once, passed to Convoy, returned to the developer, and **never stored in TripWire's database** (migration 016 dropped the `webhook_secret` column).
- Convoy is the **sole HMAC signer**. TripWire never signs outbound webhooks directly.
- The secret is **unrecoverable** after registration. If the developer loses it, they must delete the endpoint and re-register.
- A DB breach exposes zero signing secrets — they only exist in Convoy's internal storage and the developer's system.

**Future: Direct httpx delivery path (PLANNED)**

When the direct httpx fast path is implemented (bypassing Convoy for low-latency delivery), TripWire will need the raw secret at dispatch time to sign `X-TripWire-Signature` headers locally. The planned approach is **envelope encryption**:

```python
# At registration — encrypt before storing
secret = secrets.token_hex(32)
encrypted = Fernet(settings.webhook_encryption_key).encrypt(secret.encode())
# Store encrypted ciphertext in DB (new webhook_secret_encrypted column)

# At dispatch — decrypt for local HMAC signing
raw_secret = Fernet(settings.webhook_encryption_key).decrypt(encrypted)
hmac_sig = sign(payload, raw_secret)
```

- The `WEBHOOK_ENCRYPTION_KEY` (Fernet key) lives in environment secrets, never in the DB.
- A DB breach exposes only ciphertext — useless without the encryption key.
- This does NOT affect the current Convoy-only architecture. It is only needed when direct delivery is built.

### Secret Storage Summary

| Secret | Where Stored | Purpose | In DB? |
|--------|-------------|---------|--------|
| `GOLDSKY_WEBHOOK_SECRET` | Env var | Authenticate Goldsky ingest | No |
| `FACILITATOR_WEBHOOK_SECRET` | Env var | Authenticate facilitator ingest | No |
| `SUPABASE_SERVICE_ROLE_KEY` | Env var | Supabase admin access | No |
| `CONVOY_API_KEY` | Env var | Convoy API authentication | No |
| Endpoint webhook secret | Convoy internal | HMAC-sign outbound webhooks | **No** (dropped in migration 016) |
| `WEBHOOK_ENCRYPTION_KEY` | Env var (planned) | Envelope encryption for direct path | No |
