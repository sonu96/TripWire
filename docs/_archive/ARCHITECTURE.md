...
The ~100ms latency is achieved by **skipping ABI decode and finality checks** — not through a separate delivery mechanism. The facilitator sends pre-structured data (no raw log to decode), and there is no block number yet so finality is moot. A synthetic pseudo-tx-hash is generated for correlation. The pipeline still runs nonce dedup, identity resolution, policy evaluation, and dispatch via Convoy. The finality poller later promotes the event to `confirmed` once the real transaction lands.
...
- **Identity:** The `IdentityResolver` looks up the sender's ERC-8004 agent identity (see section 10).
...
Convoy is the sole HMAC signer. At endpoint creation time, TripWire passes a generated secret to Convoy, which uses it to sign every delivery. The `webhook_secret` column was removed from the `endpoints` table (migration 016) — the secret lives exclusively inside Convoy and is never stored in TripWire's database.
...
TripWire includes an optional partitioned event bus for horizontal scaling of event processing. It is **feature-flagged** via `EVENT_BUS_ENABLED` (default: `false`). When disabled, the `/ingest` endpoint processes events synchronously through `EventProcessor` with no Redis dependency.

### Data Flow (when enabled)

```
Goldsky → POST /ingest → publish_batch() → Redis Streams → TriggerWorkers → EventProcessor → Convoy/Realtime
```

Streams are keyed by `tripwire:events:{topic0}`, partitioning traffic by event type. The consumer group is `trigger-workers`.

### Key Components

| Component | File | Role |
|-----------|------|------|
| `EventBus` | `tripwire/ingestion/event_bus.py` | Redis Streams publish/consume/ack/claim primitives |
| `TriggerIndex` | `tripwire/ingestion/trigger_worker.py` | In-memory O(1) lookup by topic0, refreshes from DB every 10s |
| `TriggerWorker` | `tripwire/ingestion/trigger_worker.py` | XREADGROUP consumer loop with periodic stale-message claiming |
| `WorkerPool` | `tripwire/ingestion/trigger_worker.py` | Round-robin stream partitioning, auto-restarts crashed workers |

### Safety Mechanisms

- **DLQ:** `tripwire:dlq` stream captures messages that fail after 5 attempts (`_MAX_RETRIES`).
- **Stream cap:** `MAX_STREAMS = 500` enforced at both publish time and stream discovery time. Excess topics route to `tripwire:events:unknown`.
- **Per-worker cap:** `_MAX_STREAMS_PER_WORKER = 100` prevents a single worker from being overloaded.
- **Batch size limit:** `/ingest` rejects payloads with >1000 logs (HTTP 400).
- **Exponential backoff:** Workers sleep `min(2^n, 60)` seconds on consecutive consume errors.
- **Batched ACK:** Successful and max-retried messages are ACKed together in a single Redis pipeline.
- **Graceful startup degradation:** If the worker pool fails to start, the error is logged and the application continues without event bus workers.
...
| `EVENT_BUS_WORKERS` | `3` | Number of TriggerWorker coroutines |
...
M2M join table linking events to all endpoints they were dispatched to (migration 014). Replaces the single `endpoint_id` FK on `events` for cases where one event matches multiple endpoints.

| Field | Type | Description |
|-------|------|-------------|
| `event_id` | TEXT FK | Source event |
| `endpoint_id` | TEXT FK | Matched endpoint |
...
Deduplication table for ERC-3009 authorization nonces. Rows are reorg-aware and archived after 30 days.
...
| `reorged_at` | TIMESTAMPTZ | Set when the associated event is reorged (migration 018); null = not reorged |
| UNIQUE | | `(chain_id, nonce, authorizer)` |

Nonce rows older than 30 days are archived to a separate `nonces_archive` table (migration 019) to keep the hot table small.
...
| `topic0` | TEXT | Precomputed keccak256 hash of `event_signature` (migration 017) |
...
- Per-endpoint HMAC signing secrets are generated with `secrets.token_hex(32)` and passed to Convoy at registration time. They are not stored in TripWire's database (`webhook_secret` column was dropped in migration 016); Convoy is the sole custodian.
...
endpoints (PK: id)
    ...
    |
    |--< event_endpoints (M2M join, migration 014)
    |       |-- event_id FK, endpoint_id FK
    |
...
The MCP server uses a **3-tier auth model**:

| Tier | Mechanism | Tools |
|------|-----------|-------|
| `PUBLIC` | No auth required | `list_templates` |
| `SIWX` | SIWE wallet signature (EIP-4361) via `Authorization: Bearer <siwe_token>` | `register_middleware`, `create_trigger`, `list_triggers`, `delete_trigger`, `activate_template`, `get_trigger_status`, `search_events` |
| `X402` | x402 micropayment proof required in addition to SIWX | Tools with a non-zero price in the Bazaar manifest |

For SIWX, the server validates the EIP-4361 signed message, recovers the signer address, and uses it for ownership checks and reputation gating. Bearer tokens containing a raw Ethereum address (no signature) are rejected for SIWX-tier tools.
...
## 16. The `register_middleware` Flow
...
1. **Endpoint creation:** A new endpoint row is inserted with a nanoid, the agent's webhook URL, delivery mode, chain IDs, recipient address (defaults to agent address), and endpoint policies. A webhook secret is generated and passed to Convoy for HMAC signing, but is not stored in the database.
...
4. **Response:** Returns `endpoint_id`, all `trigger_ids`, `mode`, and `url`. The middleware is now live -- incoming events matching these triggers will be decoded, filtered, and dispatched to the agent's URL.
...
## 17. x402 Bazaar and Service Discovery
...
2. **Register:** Agent calls `register_middleware` via MCP, creating an endpoint and triggers (from templates or custom definitions). Receives `endpoint_id` and `trigger_ids`.
